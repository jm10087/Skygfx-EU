...
InjectDelayedPatches()
{
	if(!IsAlreadyRunning()){
		...
		if(disableGamma)
			MemoryVP::InjectHook(0x74726C, 0x747343, PATCH_JUMP); //different
...
WRAPPER void gtasrand(unsigned int seed) { EAXJMP(0x821B51); } //different
...
DllMain(HINSTANCE hInst, DWORD reason, LPVOID)
{
	if(reason == DLL_PROCESS_ATTACH){
		...
		if(*(DWORD*)DynBaseAddress(0x8245BC) != 0x94BF &&
		   *(DWORD*)DynBaseAddress(0x82457C) == 0x94BF)
			return FALSE;
		dllModule = hInst;
		...
		IsAlreadyRunning = (BOOL(*)())(*(int*)(0x74877D+1) + 0x74877D + 5); //different
		MemoryVP::InjectHook(0x74877D, InjectDelayedPatches); //different

		MemoryVP::InjectHook(0x5BCF14, afterStreamIni, PATCH_JUMP);

		MemoryVP::InjectHook(0x749210, myDefaultCallback, PATCH_JUMP); //different

		MemoryVP::InjectHook(0x5BF8EA, CPlantMgr_Initialise);
		MemoryVP::InjectHook(0x756E4E, rxD3D9DefaultRenderCallback_Hook, PATCH_JUMP); //different
		...
		// postfx
		MemoryVP::InjectHook(0x74EAF6, CPostEffects::Init, PATCH_JUMP); //different
